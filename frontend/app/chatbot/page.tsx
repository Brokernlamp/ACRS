...
interface Status { gemini_configured: boolean; rag_documents_indexed: number; ready: boolean }
...
export default function ChatbotPage() {
  const [messages, setMessages] = useState<Message[]>([{
    role: "model",
    content: "Hi! I'm your AI marketing analyst. I have full context of your campaign data — ask me anything about performance, budgets, predictions, or recommendations.",
  }]);
  ...
  useEffect(() => {
    api.chatStatus().then(s => setStatus(s as unknown as Status)).catch(() => setStatus(null));
  }, []);
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      api.chatStatus().then(s => setStatus(s as unknown as Status)).catch(() => {});
    } catch (e: unknown) {
      setMessages(prev => [...prev, { role: "model", content: `⚠️ ${e instanceof Error ? e.message : "Failed to get response"}` }]);
    } finally { setLoading(false); }
  }

  async function handleReset() {
    await api.chatReset().catch(() => {});
    setMessages([{ role: "model", content: "Conversation cleared. Ask me anything about your campaign data!" }]);
  }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3rem)] max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={handleReset} className="flex items-center gap-2 text-sm text-gray-500 hover:text-red-600 border border-gray-200 hover:border-red-300 px-3 py-1.5 rounded-lg transition-colors">
      ...
      {status && !status.ready && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-800 text-sm px-4 py-3 rounded-lg mb-3">
          ...
          <div className="space-y-1">
            {!status.rag_documents_indexed && <p><strong>No data indexed.</strong> Upload a CSV on the Dashboard first.</p>}
      ...
      {status?.ready && (
        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs px-4 py-2 rounded-lg mb-3">
          <CheckCircle size={13} />
          {status.rag_documents_indexed} documents indexed · {((status as unknown as Record<string, unknown>)["active_provider"] as string) !== "none" ? `${(status as unknown as Record<string, unknown>)["active_provider"]} connected` : "fallback mode"}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto space-y-4 bg-white border border-gray-200 rounded-xl p-4 mb-4">
        ...
        {loading && (
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center"><Bot size={15} className="text-gray-600" /></div>
